...
Because the parameter \texttt{x} can be of any subclass of \texttt{List} we can not for sure know the implementation of \texttt{x.Add()}, nor therefore can we be certain of \texttt{x.Add()}'s purity. Thus, a static analysis method can not for sure determine \texttt{f()}'s purity by only looking at its definition.
...
\begin{figure}[H]
  ...
  \end{lstlisting}
  % \caption{\texttt{List.Add()} has a side effect because it modifies the \texttt{list}, which was declared before calling the method. However, since \texttt{list} is fresh, \texttt{Foo()}'s purity is not affected.}
