...
\begin{itemize}
  ...
  \item If the analyzed method \textit{m} modifies an input parameter of reference type, mark \textit{m} as parametrically impure. This could be done either by mutating a parameter object's field, a parameter object's property or the cell of a parameter array, or by calling a locally impure method belonging to an object type parameter.
  \item If the currently analyzed method \textit{m} is overridden by any impure, locally impure and/or parametrically impure method, mark \textit{m} with the impurity or impurities of the overriding method or methods.
    % TODO: shouldn't we rather check if the method called on a parameter is overridden? I.e. if a parameter's method is called, require all implementations of it to be pure
