...
\begin{itemize}
  ...
  \item Methods for how to deal with a number of problems that arise when measuring purity in an object-oriented programming language.
\end{itemize}

The next section contains definitions for functional purity, functional programming and object oriented programming. \Autoref{sec:the-csharp-language} describes C\# and .NET, as well was a description of what makes C\# method pure. \Autoref{sec:problems-with-purity-in-oo} presents a number of dilemmas when it comes to measuring purity in object-oriented languages, and how they were addressed. \Autoref{sec:the-analysis-method} describes the analysis method used, and applies it to a simple C\# program. \Autoref{sec:implementation-of-cspurity} describes the implemented prototype of the analysis method -- CsPurity. \Autoref{sec:results-and-discussion} presents the results after running CsPurity on the example code in \autoref{sec:the-analysis-method} as well as on 11 open source repositories that use the \texttt{[Pure]} attribute, which was used to evaluate CsPuritiy's precision and recall. The results are then discussed at the end of \autoref{sec:results-and-discussion}, and the thesis is concluded in \autoref{sec:conclusion}.

\Autoref{sec:related-work} presents similar work done in other programming languages.

TODO: fix this paragraph

\section{Related work} \label{sec:related-work} % (If not part of introduction)

The problem of detecting functional purity has been studied in other programming languages. Even though each language has its unique features there is a lot to be learned from how others have approached the subject. This section describes related work done for purity analysis in Erlang as well as Java. Since Java is object-oriented, it is relatively similar to C\#, and so four out of the five related works cover purity analysis in Java.

Not all literature defines functional purity the same, as will become clear in this section. Each subsection includes that work's definition. The definition for functional purity used in this thesis, as well its motivation, is in \autoref{sub:definitions}.
...
Their analysis starts with Erlang's so called built-in-functions (BIFs), which are functions native to the Erlang's virtual machine and are written in C~\cite{pitidis2010purity}. Impure actions in Erlang can only be done through BIFs, including performing I/O actions or writing to global variables~\cite{kostis-email}. Because BIFs are written in C they cannot be analyzed by their analysis tool, and their purity is assumed to be already known beforehand by the analysis tool~\cite{pitidis2010purity}. The analysis propagates the impurity of BIFs to each function which directly or indirectly depends on them.
...
For example, to normalize the amount of \texttt{[Pure]}s classified as \textit{pure}, for each repository \textit{R}, the number of methods in \textit{R} classified as \textit{pure} is divided by the total amount of \texttt{[Pure]} methods in \textit{R}. This is done for all the combinations of in the \texttt{[Pure]} or non-\texttt{[Pure]} with all

This is done for each of CsPurity's classified purity level in both \autoref{tab:results-pure-methods} and \autoref{tab:results-non-pure-methods}.

This yields the precision and recall shown in \autoref{tab:results-precision-recall-normalized}, which demonstrates significantly better performance.
