...
\subsubsection{\texttt{encoding.cs}}

\textbf{With \texttt{[Pure]} attributes}

\begin{itemize}
  \item Pure: 2
  \item Impure: 13
  \begin{itemize}
    \item Throws exception: 13
    \item Other impure action: 0
  \end{itemize}
  \item Unknown: 11
\end{itemize}

\textbf{Without \texttt{[Pure]} attributes}
...
Although the majority of methods are \textit{impure} all of those methods throw exceptions.

All seven methods categorized as \textit{pure} by CsPurity but with no \texttt{[Pure]} attribute are in fact pure. Three of them write to \texttt{internal} fields, have the \texttt{unsafe} keyword and two simply return fresh objects.
