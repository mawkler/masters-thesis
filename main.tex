...
\subsection{Results from scanning the example} \label{sub:results-from-scanning-the-example}
...
\begin{figure}[htpb]
  ...
  \begin{minipage}{\linewidth} % Forces code to be on same page
    ...
    \end{lstlisting}
  \end{minipage}
  \caption{Result from running the analyzer on the implementation of \texttt{LinkedList} in \autoref{fig:example-code}.}
...
However, the three other analyzed methods \texttt{Length()}, \texttt{Add()} and \texttt{Remove()}'s purity levels -- which are always initialized to \textit{pure} in the lookup table -- were unaffected by the analysis. As seen in \autoref{tab:lookup-table3}, \texttt{Length()}'s purity level should be unaffected and remain \textit{pure}. However,
