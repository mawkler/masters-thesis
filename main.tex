...
\begin{itemize}
  \item There are three kinds of purity levels: \textit{pure}, \textit{impure}, and \textit{uknown}.
...
If we modify a list that was created inside a method \textit{m}, say by calling the function \texttt{append()}, \textit{m} will look like it has a side-effect, even though it hasn't.

Solution: if a called method \textit{m} is impure, only mark \textit{m}'s callee \textit{M} as impure if the object which \textit{m} belongs to was not created inside the callee \textit{M}. This means that if \textit{m}
...
Solution: Add additional impurity level: \textit{Locally impure} -- methods that have no side-effects besides modifying their input parameters.

A method \textit{m} that calls a locally impure method of an object is pure if the object was created by \textit{m}.
