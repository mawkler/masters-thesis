...
\begin{itemize}
  ...
  \item If the currently analyzed method \textit{m} calls an input parameter's method $m_p$ and $m_p$ is overridden by any impure, locally impure and/or parametrically impure method, mark $m_p$ with the impurity or impurities of the overriding method or methods. % Moreover:
    ...
    % \end{itemize}
  \item If the analyzed method \textit{m} modifies an input parameter of reference type, mark \textit{m} as parametrically impure. This could be done either by directly mutating a parameter object's field, a parameter object's property or the cell of a parameter array, by calling a % (overriding comes in here)
    method that has been marked locally impure and that belongs to an object type parameter (i.e. the called method may have been overridden by a locally impure method as mentioned previously), or by passing a parameter as an argument to a parametrically impure method. I.e.:
    \begin{itemize}
      \item If any of \textit{m}'s input parameters are passed as an argument to a parametrically impure method, then \textit{m} is parametrically impure
      \item
    \end{itemize}
  ...
  \item If a the method \textit{m} calls a locally impure method belonging to the same object as \textit{m} (i.e. \texttt{this}) \textit{m} is also impure
\end{itemize}
...
% We do however have to check if methods belonging to input parameters are impure or locally impure


% Method m is marked parametrically impure if it:
% - Modifies a value of a reference type input parameter
%   - The field of an object
%   - Or the cell of an array
% - Modifies an input parameter object by calling a method belonging to it that is locally impure
%   - If at least one of the overridden methods is locally impure, then m is parametrically impure
