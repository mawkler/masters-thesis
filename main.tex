...
\begin{table}[H]
  ...
  \end{tabular}
  \\
  $W$ = \{\texttt{Add()}, \texttt{Concatenate$_2()$}\}
\end{table}

Now we perform the same actions again with the new working set $W$ = \{\texttt{Add()}, \texttt{Concatenate$_2()$}\}, starting with \texttt{Add()}. Out of the items in the checklist in \autoref{sub:checklist-to-determine-the-purity-level}, \autoref{itm:locally-impure} applies to \texttt{Add()}, since it modifies its object in multiple locations, for instance by assigning to the field \texttt{tail}, and so \texttt{Add()}'s purity level is set to \textit{locall impure}.

As for \texttt{Concatenate$_2()$}, none of the items in the checklist apply, and so it remains its purity level \textit{impure} that was propagated to it from \texttt{Concatenate$_1()$}.

\texttt{Add()} and \texttt{Concatenate$_2()$} are then both removed from $W$.

\begin{table}[H]
  \caption{\texttt{Add()}'s purity level has been updated to \textit{locally impure}, and $W$ is now empty. Therefore, this is the result of the analysis.}
