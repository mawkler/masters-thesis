...
When calling an argument's method, because of inheritance and method overriding we can never be sure of which method implementation will be called. Consdier the following example~\cite{pearce2011jpure}:
...
\begin{figure}[H]
  ...
  \end{lstlisting}
  \caption{Because \texttt{x} can be any subclass of \texttt{List} we can never be sure of \texttt{add()}'s implementation}
  \label{fig:inheritance-example}
\end{figure}

Because \texttt{x} can be any subclass of \texttt{List} we can not for sure know the implementation of \texttt{x.add()}, and therefore nor can we be certain of \texttt{x.add()}'s purity. Thus, we can not determine \texttt{f()}'s purity.

One solution to this that David J. Pearce suggests is to demand that pure methods can only be overridden by methods that are also pure~\cite{pearce2011jpure}.

\textbf{Iterators} % Not relevant because my definition of purity includes determinism
As Pearce suggests, an iterator may at first glance look pure~\cite{pearce2011jpure}. Consider the following:
...
If we modify a list that was created inside a method \textit{m}, say by calling the function \texttt{list.append()}, \textit{m} may look like it has a side-effect, even though it hasn't.
