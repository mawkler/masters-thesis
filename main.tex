...
\subsection{Results from running CsPurity larger codebase} \label{sub:results-larger-codebase}

As mentioned in \autoref{sub:code-contracts} the \texttt{[Pure]} attribute can be placed in front of a method definition to signal that the method pure. That means that running CsPurity on a code base where a portion of the methods use the \texttt{[Pure]} attribute CsPurity's accuracy can be evaluated by comparing what the methods with and the methods without the \texttt{[Pure]} attribute were categorized as by CsPurity.
It is assumed that any method with the \texttt{[Pure]} attribute is functionality pure. This does not neccessarily have to be true in general, and as will be discussed later, a large portion of \texttt{[Pure]} methods do in fact have side-effects. But the \texttt{[Pure]} attribute can still tell us something about how CsPurity performs.
The evaluation also assumes that methods without \texttt{[Pure]} are impure. There could be exceptions to this, i.e. pure methods that haven't been given the \texttt{[Pure]} attribute by the code's author, and this will be discussed further later in this section. % TODO
However, this assumption still allows us to also say something about CsPurity ability to classify impure methods.

To evaluate CsPurity's accuracy the 14 most popular GitHub repositories that use C\# and the \texttt{[Pure]} attribute use were chosen. Here "popular" is synonymous with the number of stars, which is GitHub's user rating system for repositories.

The results are divided into two different tables; \autoref{tab:results-pure-methods} which contains CsPurity's classification for methods with the \texttt{[Pure]} attribute, and \autoref{tab:results-non-pure-methods} with the result for methods without the attribute. Methods with the \texttt{[Pure]} attribute make up 37\% percent of the total number of methods analyzed, and so 63\% of methods do not have the attribute.

\begin{table}[H]
  \small
  \caption{CsPurity's classification of the methods with the \texttt{[Pure]} attribute after being run on 14 different open source repositories.}
  ...
  \begin{tabular}{|l|lp{2.8cm}p{1.4cm}l|l|}
    ...
    gitextensions                    & 1   & 5    & 14    & 27    & 47    \\
    MetadataExtractor .NET           & 2   & 9    & 9     & 43    & 63    \\
    Spreads                          & 0   & 31   & 3     & 1     & 35    \\
    fluentassertions                 & 53  & 0    & 4     & 20    & 77    \\
    \hline
    \rowcolor{lightgray} \textbf{Total number of methods} & 938 & 1357 & 16858 & 26404 & \cellcolor{white} 45557 \\
...
\begin{table}[H]
  \small
  \caption{CsPurity's classification of the methods with no \texttt{[Pure]} attribute after being run on 14 different open source repositories.}
  ...
  \begin{tabular}{|l|lll|l|}
    ...
    .NETOpenAuth                     & 1438 & 2099  & 2350  & 5887  \\
    CsConsoleFormat                  & 35   & 272   & 576   & 883   \\
    Bridge                           & 1654 & 3733  & 8007  & 13394 \\
    ...
    gitextensions                    & 320  & 2618  & 4405  & 7343  \\
    MetadataExtractor .NET           & 36   & 1267  & 341   & 1644  \\
    Spreads                          & 261  & 1094  & 546   & 1901  \\
    fluentassertions                 & 703  & 3771  & 1114  & 5588  \\
    \hline
    \rowcolor{lightgray} \textbf{Total number of methods} & 7379 & 38975 & 32037 & \cellcolor{white} 78391 \\
    \rowcolor{lightgray} \textbf{Total percentage}        & 9\%  & 50\%  & 41\%  & \cellcolor{white} 100\% \\
    \hline
  \end{tabular}
\end{table}

% Reference Source is a publicly available repository by Microsoft that contains sources from the Microsoft .NET Reference Source~\cite{microsoft-referencesource}. Their \texttt{String} and \texttt{Encoding} classes together contain 91 methods with the \texttt{[Pure]} attribute out of a total of 177 methods. By scanning methods that are marked with the \texttt{[Pure]} attribute we can determine the accuracy of CsPurity.

% However, Microsoft don't seem to count reading of static variables (non-determinism) or calls to methods that throw exceptions as impure. Therefore, the majority of methods with the \texttt{[Pure]} atttribute are in fact impure.

% The two classes in Reference Source with the most number of methods with a \texttt{[Pure]} attribute were chosen.

% Out of all 91 methods that were marked as \texttt{[Pure]}:

% \begin{itemize}
%   \item Pure: 4
%   \item Impure: 55, where
%   \begin{itemize}
%     \item Throw exception: 46
%     \item Other impure action: 9
%   \end{itemize}
%   \item Unknown: 32
% \end{itemize}
...
% Out of all 177 methods that were \textit{not} marked as \texttt{[Pure]}:
...
% Note that as mentioned in \autoref{sub:code-contracts} the \texttt{[Pure]} attribute only requires side-effect freeness, not determinism. This is the reason why the majority of \texttt{[Pure]} methods were marked as \textit{impure} by CsPurity.

% \textbf{string.cs}

% Nine methods in the \textit{String} class had a \texttt{Pure} attribute but were classified as \textit{impure} by CsPurity. All of them are non-deterministic since they read static variables.
...
% Although the majority of methods with \texttt{[Pure]} attribute were classified as \textit{impure} by CsPurity, all of them throw exceptions.

% Seven methods that were categorized as \textit{pure} by CsPurity but with no \texttt{[Pure]} attribute are upon inspection in fact pure. % Three of them write to \texttt{internal} fields, have the \texttt{unsafe} keyword and two simply return fresh objects.

% Although there were some apparent false negatives where CsPurity categorized \texttt{[Pure]} methods as impure. However, they all seem to be due to Microsoft's looser definition of functional purity which doesn't include determinism or exceptions. The majority of false negatives were \texttt{[Pure]} methods that throw errors, and in some cases they read or even modify a static field, or depend on a method that does one of these things. Not one case was found where CsPurity's categorization was done incorrectly based on the definition used in this thesis.
