...
\subsubsection{Modifying a value received from constructor} \label{sub:Modifying a value received from constructor}
\begin{figure}[H]
  \centering
  \begin{lstlisting}
    public class Program {
      ...
      public void Foo() {
        A b = new A(myList);
        b.Bar(); // calling b.Bar() will cause myList to change
      }
      ...
      public class A {
        ...
        public void Bar() {
          list.Add("foo"); // looks locally impure, but it modifies constructor object
        }
      }
    }
  \end{lstlisting}
  \caption{\texttt{ListHas()} may at first glance look pure}
  \label{fig:constructor-value-example}
\end{figure}
...
Methods are marked parametrically impure if they:
- Modify a field of some a parameter
- Modify an input parameter by calling a locally impure method on it
