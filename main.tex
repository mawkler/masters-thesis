...
As discussed in \autoref{sub:code-contracts}, Microsoft's definition of functional purity used for the \texttt{[Pure]} attribute does not include determinism like the definition used in this thesis, \autoref{def:functional-purity}, does. That means that methods that read or modify static fields, or that throw exceptions do not violate the \texttt{[Pure]} attribte. This was confirmed to be the case when looking manually at smaller samples of methods in each repository as no incorrectly classified method based on \autoref{def:functional-purity} was found. This may explain why 3\% + 37\% = 40\% of methods in \autoref{tab:results-pure-methods} were classified by CsPurity as \textit{impure} ("Impure: throws exception" and "Impure: other" in the table), despite having the \texttt{[Pure]} attribute. These 40\% of all \texttt{[Pure]} methods also directly contribute to the low recall for classification of pure methods, as well as the relatively low precision for impure classification seen in \autoref{tab:results-precision-recall}.

Moreover, the difference in definition of functional purity between CsPurity and the \texttt{[Pure]} attribute, together with almost 60\% \texttt{[Pure]} \textit{unknowns} significantly impacts the low amount of true positives when it comes to pure methods. As seen in \autoref{tab:results-pure-methods} only 2\% of \texttt{[Pure]} methods were classified as \textit{pure} by CsPurity.

As mentioned earlier in this section it is assumed that any method without a \texttt{[Pure]} attribute is impure. While this is a necessary assumption in order to be able to say anything about CsPurity's ability to classify impure methods, this is not always the case. This is also reflected in the 8\% of methods in \autoref{tab:results-non-pure-methods} that were classified as \textit{pure} by CsPurity but that have no \texttt{[Pure]} attribute. 8\% may not sound like much but in combination with the amount of \texttt{[Pure]} methods classified as \textit{pure} at only 2\% significantly impacts the precision of classification of pure methods.

This contributes somewhat (or a lot?) to

Similarly, when looking at the precision of classifying pure methods and the recall for impure ones...
